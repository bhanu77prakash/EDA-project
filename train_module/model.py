...
class ModuleNet(nn.Module):
  def __init__(self, vocab, feature_dim=(1024, 14, 14),
    ...
    self.stem = build_stem(feature_dim[0], module_dim,num_layers=stem_num_layers,with_batchnorm=stem_batchnorm)
    ...
    for fn_str in vocab['program_token_to_idx']:
      ...
      if fn_str == 'scene' or num_inputs == 1:
        ...
        mod = ConcatBlock(module_dim,with_residual=module_residual,with_batchnorm=module_batchnorm)
...
class Seq2Seq(nn.Module):
  def __init__(self,
    ...
    self.encoder_rnn = nn.LSTM(wordvec_dim, hidden_dim, rnn_num_layers,dropout=rnn_dropout, batch_first=True)
    self.decoder_embed = nn.Embedding(decoder_vocab_size, wordvec_dim)
    self.decoder_rnn = nn.LSTM(wordvec_dim + hidden_dim, hidden_dim, rnn_num_layers,dropout=rnn_dropout, batch_first=True)
